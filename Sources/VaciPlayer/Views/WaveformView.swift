                ...
                if isLoading {
                    ...
                    GeometryReader { geometry in
                        ...
                        Canvas { context, size in
                            ...
                            let barWidth = zoomedWidth / CGFloat(barCount)
                            let centerY = size.height / 2

                            // Draw loop selection background
                            if let start = loopStart, let end = loopEnd, song.duration > 0 {
                                ...
                                context.fill(Path(loopRect), with: .color(.blue.opacity(0.2)))
                            }

                            // Draw current drag selection
                            if let ds = dragStart, let de = dragEnd {
                                let startX = min(ds.x, de.x)
                                let endX = max(ds.x, de.x)
                                let dragRect = CGRect(
                                    x: startX,
                                    y: 0,
                                    width: endX - startX,
                                ...
                                context.fill(Path(dragRect), with: .color(.blue.opacity(0.3)))
                            }

                            // Draw waveform bars - single draw pass
                            let hasLoop = loopStart != nil && loopEnd != nil && song.duration > 0

                            for i in 0..<barCount {
                                let x = CGFloat(i) * barWidth + clampedPanOffset

                                // Skip bars outside visible area
                                guard x + barWidth > 0 && x < size.width else { continue }

                                let amplitude = CGFloat(waveformData[i])
                                let barHeight = amplitude * size.height * 0.8

                                // Determine bar color
                                var barColor: Color = .primary
                                if hasLoop, let ls = loopStart, let le = loopEnd {
                                    let barTime = Double(i) / Double(barCount) * song.duration
                                    if barTime >= ls && barTime <= le {
                                        barColor = .blue
                                    }
                                }
                            ...
                            if song.duration > 0 {
                                ...
                                if currentX >= 0 && currentX <= size.width {
                                    let positionRect = CGRect(x: currentX - 1, y: 0, width: 2, height: size.height)
                                    context.fill(Path(positionRect), with: .color(.red))
                                }
                            }
                        }
                        .clipShape(RoundedRectangle(cornerRadius: 8))
                        ...
                        .gesture(
                            MagnificationGesture()
                                .onChanged { value in
                                    let newZoom = max(minZoom, min(maxZoom, zoomLevel * value))
                                    zoomLevel = newZoom
                                }
                        )
                    }
